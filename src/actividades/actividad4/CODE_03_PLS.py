# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 16:48:34 2023
...
data = pd.read_csv('datos_PLS.txt')
...
# Function to create heatmap with labels
def create_heatmap(corr_matrix, labels, title):
    plt.figure(figsize=(10, 8))
    sns.heatmap(corr_matrix, annot=True, fmt=".2f", cmap='coolwarm', cbar=True, xticklabels=labels, yticklabels=labels)
    plt.title(title)
    plt.show()

# Create heatmaps with labels
create_heatmap(corr_origin, list(data.columns), 'Correlation Matrix - Original Data')
create_heatmap(corr_pca, list(data_pca.columns), 'Correlation Matrix - PCA Data')
create_heatmap(corr_pls, list(data_pls.columns), 'Correlation Matrix - PLS Data')